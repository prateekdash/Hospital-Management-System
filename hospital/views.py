...
def Add_Doctor(request):
    ...
    if request.method == "POST":
        ...
        ex = request.POST['experties']
        
        try:
            Doctor.objects.create(name=n, mobile=m, experties=ex)
            error = "no"
        except:
            error = "yes"
    d = {'error':error}
...
def Take_Appointment(request):
    ...
    if request.method == "POST":
        ...
        t = request.POST['time']
        doctor=Doctor.objects.filter(name=do).first()
    ...
    return render(request,'take_appointment.html', d)   
